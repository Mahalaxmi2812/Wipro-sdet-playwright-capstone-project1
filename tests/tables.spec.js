...
test.describe('Tables — Page Load', () => {
  ...
  test('TC04-TC06 - employee list has all search controls visible and enabled', async ({ employeeListPage }) => {
    ...
    await expect.soft(employeeListPage.paginationList).toBeVisible();
...
test.describe('Tables — Pagination', () => {
  ...
  test('TC10 - more than one page exists', async ({ employeeListPage }) => {
...
test.describe('Tables — Filtering', () => {
  ...
  test('TC19 - employee table has correct structure with headers, rows and pagination', async ({ employeeListPage }) => {
    ...
    await expect.soft(employeeListPage.selectedPageBtn).toHaveText('1');
    expect.soft(headerCount).toBeGreaterThanOrEqual(5);
    expect.soft(rowCount).toBeGreaterThanOrEqual(10);
    expect(test.info().errors).toHaveLength(0);
  });
});
